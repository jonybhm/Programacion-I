...
class Tablero:
        ...
        for x in range(0,CANTIDAD_TARJETAS_H*ANCHO_TARJETA,ANCHO_TARJETA):
            for y in range(0,CANTIDAD_TARJETAS_V*ALTO_TARJETA,ALTO_TARJETA):
                if(i > CANTIDAD_TARJETAS_UNICAS):
                    tarjeta_test = Tarjeta()
                    tarjeta_test.path_imagen = "0{0}.png".format(i-CANTIDAD_TARJETAS_UNICAS)
                    tarjeta_test.path_imagen_hide = r"00.png"
                    tarjeta_test.rectx()
                    tarjeta_test.recty()
                else:
                    tarjeta_test = Tarjeta()                    
                    tarjeta_test.path_imagen = "0{0}.png".format(i)
                    tarjeta_test.path_imagen_hide = r"00.png"
                    tarjeta_test.rect.x = x
                    tarjeta_test.rect.y = y
    ...
    def update(self):
        ...
        if(self.tiempo_ultimo_destape > 0):
            if(tarjeta.match(self.l_tarjetas)):
                self.tiempo_ultimo_destape = 0
